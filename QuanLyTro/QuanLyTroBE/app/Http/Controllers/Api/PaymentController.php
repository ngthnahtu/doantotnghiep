...
class PaymentController extends Controller
{
    ...
    public function index()
    {
        $user = Auth::user();
        if ($user->role === 0) {
            $payments = Payment::with('invoices.contracts.tenants','invoices.rooms')->orderBy('created_at', 'desc')->paginate(8);
        } else {
            $tenant = $user->tenants;
            if (!$tenant) {
                return response()->json(['message' => 'Không tìm thấy hồ sơ khách thuê.'], 404);
            }
            $payments = Payment::with('invoices.contracts.tenants','invoices.rooms')->whereHas('invoices.contracts', function ($query) use ($tenant) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($hasPayment){
                return response()->json([
                    'message' =>'Hóa đơn này đã có giao dịch đang chờ duyệt.'
                ], 422);
            }

            if ($invoice->remain_amount <= 0) {
                return response()->json([
                    'message' => 'Hóa đơn này không còn số tiền cần thanh toán.'
                ], 422);
            }

            if ($user->role === 1) {
                $tenant = $user->tenants;
                if (!$tenant || !$invoice->contracts || $invoice->contracts->tenant_id !== $tenant->id) {
                    return response()->json(['message' => 'Bạn không có quyền thực hiện thanh toán cho hóa đơn này.'], 403);
                }
            ...
            if ($invoice->status == 3) {
                return response()->json([
                    'message' => 'Hóa đơn này đã hoàn tất đóng tiền, không thể tạo thêm giao dịch.'
                ], 422);
            }

            if ($validated['amount'] > $invoice->remain_amount) {
                return response()->json([
                    'message' => 'Số tiền thanh toán vượt quá số tiền còn nợ thực tế của hóa đơn',
                ], 422);
            }
            ...
            $payment = DB::transaction(function () use ($validated, $request,$invoice,$isAdminCashPayment) {
                ...
                $payment = Payment::create([
                    ...
                    'approved_at'=>$isAdminCashPayment ? $validated['payment_date'] : null,
    ...
    public function update(Request $request, string $id)
    {
        ...
        $payment = Payment::with('invoices.contracts.tenants')->find($id);
        if (!$payment) {
            return response()->json(['message' => 'Không tìm thấy thông tin thanh toán này.'], 404);
        }

        if ($payment->status !== 0) {
            return response()->json(['message' => 'Giao dịch này đã được xử lý trước đó, không thể chỉnh sửa.'], 422);
        }
            ...
            DB::transaction(function () use ($payment, $validated) {
                ...
                if ($validated['status'] === 1) {
                    ...
                    if ($payment->amount > $invoice->remain_amount) {
                        throw new \Exception(
                            'Số tiền giao dịch vượt quá số tiền hóa đơn còn nợ.'
                        );
                    }
                    $paidAmount = $invoice->paid_amount + $payment->amount;
                    $remainAmount = $invoice->total_amount - $paidAmount;
