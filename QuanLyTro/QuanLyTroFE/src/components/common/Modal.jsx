...
function Modal({ title, isOpen, onClose, children, className = "max-w-lg" }) {
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full h-screen flex items-center justify-center bg-black/30">
      <div
        className={`overflow-y-auto max-h-[calc(100dvh-24px)] w-full rounded-xl bg-white shadow p-6 transition-all duration-200 
          dark:bg-slate-900 dark:text-slate-100 ${className}`}
      >
