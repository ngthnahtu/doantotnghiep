...
const invoiceStatus = ["Chờ thanh toán", "Chờ duyệt", "Thanh toán một phần", "Hoàn thành", "Quá hạn"];
...
export default function Dashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      setToast({
        type: "error",
        message: error.response?.data?.message || "Không thể tải dữ liệu tổng quan.",
      });
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div className="flex items-center justify-between border-b border-slate-200 p-4 dark:border-slate-700">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-semibold text-slate-800 dark:text-slate-100">Hóa đơn gần nhất</h2>
                  </div>

                  <span className="text-sm text-slate-400">Mới nhất</span>
                </div>

                <div className="px-4">
                  {!dashboard?.invoice_recent?.length ? (
                    <p className="py-10 text-center text-sm text-slate-500 dark:text-slate-400">Chưa có hóa đơn.</p>
                  ) : (
                      ...
                      >
                        ...
                        <div className="ml-4 text-right">
                          ...
                          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            {invoiceStatus[Number(invoice.status)] || "Không xác định"}
                          </p>
                        </div>
                      </div>
                    ))
              ...
              <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div className="flex items-center justify-between border-b border-slate-200 p-4 dark:border-slate-700">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-semibold text-slate-800 dark:text-slate-100">Sự cố gần nhất</h2>
                  </div>
                ...
                <div className="px-4">
                  {!dashboard?.issue_recent?.length ? (
                    <p className="py-10 text-center text-sm text-slate-500 dark:text-slate-400">Chưa có sự cố.</p>
                  ) : (
                    dashboard.issue_recent.map((issue) => {
                      ...
                      return (
                        ...
                        >
                          <div className="min-w-0">
                            <p className="truncate font-medium text-slate-800 dark:text-slate-100" title={issue.title}>
                              {issue.title}
                            </p>
...
function DashboardCard({ title, value, icon: Icon, color, onClick }) {
  return (
    <div
      ...
      dark:border-slate-700 dark:bg-slate-900 ${onClick ? "hover:shadow-md hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer" : ""}`}
      onClick={onClick}
    >
      <div className="min-w-0">
        <p className="text-sm text-slate-500 dark:text-slate-400">{title}</p>

        <p className="mt-1 truncate text-2xl font-semibold text-slate-800 dark:text-slate-100">{value}</p>
        {onClick && (
